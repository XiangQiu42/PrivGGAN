...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=False):
    ...
    print(f"The num of graphs in the original/generated graphs: {len(sample_ref)}/{len(sample_pred)}")
...
def orca(graph):
    ...
    # Note: orca.exe file is suitable for windows, for Linux, one need to use the 'orca' file, e.g.
    # output = sp.check_output(['./evaluation/orca/orca', 'node', '4', 'evaluation/orca/tmp.txt', 'std'])
    output = sp.check_output(['./evaluation/orca/orca.exe', 'node', '4', 'evaluation/orca/tmp.txt', 'std'])
